...
- Delete line at caret **`Ctrl+Y`**, **`Command+BackSpace`**
- Smart line join **`Ctrl+Shift+J`**, **`Ctrl+Shift+J`**
- <ins>Move line up/down</ins> **`Shift+Alt+↑/↓`**, **`Shift+Option+↑/↓`**
- <ins>Move statement up/down</ins> **`Shift+Ctrl+↑/↓`**, **`Shift+Command+↑/↓`**
- <ins>Move element left/right</ins> **`Shift+Ctrl+Alt+←/→`**, **`Command+Shift+Option+←/→`**

### 3. 코드 즉시보기
- Parameter info(within method call arguments) **`Ctrl+P`**, **`Command+P`**
- Open quick definition lookup **`Ctrl+Shift+I`**, **`Option+Space`**
- Quick documentation lookup **`Ctrl+Q`**, **`F1`**

> - Ultimate 버전에서는 Quick Definition, Documentation를 web 파일들에도 사용 가능
>     - JS파일의 경우 MDN Doc 내용이 JS의 문서로 나옴

## [2] 포커스
### 1. 포커스 에디터
- <ins>Move caret to next word</ins> **`Ctrl+←/→`**, **`Option+←/→`**
- <ins>Move caret to next word with selection</ins> **`Shift+Ctrl+←/→`**, **`Shift+Option+←/→`**
- <ins>Move caret to start/end of line</ins> **`Home/End`**, **`Fn+←/→`**
- <ins>Move caret to start/end of line with selection</ins> **`Shift+Home/End`**, **`Command+Shift+←/→`**
- <ins>Move caret to page end</ins> **`PageUp/PageDown`**, **`Fn+↑/↓`**

### 2. 포커스 특수키
- Extend selection **`Ctrl+W`**, **`Option+↑`**
- Shrink selection **`Shift+Ctrl+W`**, **`Option+↓`**
- Navigate back/forward **`Ctrl+Alt+←/→`**, **`Command+[/]`**
- <ins>Multi cursor</ins> **`Ctrl+Ctrl+↑/↓`**, **`Option+Option+↑/↓`**
...
- <ins>Regex search/replace</ins> - Regex checkbox
    - `\/tmfdk333\/([A-Za-z]*)\/app.js` → `\/inflearn\/$1\/app.js`
...
- <ins>Recently change files</ins> **`Ctrl+Shift+E`**, **`Command+Shift+E`**

## [4] 자동완성
### 1. 자동완성
- Basic code completion **`Ctrl+Space`**, **`Ctrl+Space`**
- Smart code completion **`Ctrl+Shift+Space`**, **`Ctrl+Shift+Space`**
- Show intention actions and quick-fixes **`Alt+Enter`**, **`Option+Enter`**
- <ins>Static method completion</ins> **`Ctrl+Space*2`**, **`Ctrl+Space*2`**
- Generate code **`Alt+Insert`**, **`Command+N`**
...
- Custom live template
    - Ctrl/Command+Shift+A → live templates|Settings → other → + 
...
- Extract Parameter **`Ctrl+Alt+P`**, **`Command+Option+P`**
    ...
    - Delegate via overloading method 
        - Overloading 메소드를 하나 더 만들어서 파라미터로 추출 
- Extract Method **`Ctrl+Alt+M`**, **`Command+Option+M`**
- Move **`F6`**, **`F6`** - 이너클래스 추출하려면 Focus를 클래스명으로 옮겨야 함
 
### 2. 리팩토링 기타
- Rename **`Shift+F6`**, **`Shift+F6`** - Class, Method, Variable, Field, Parameter
- <ins>Type Migration</ins> **`Ctrl+Shift+F6`**, **`Command+Shift+F6`**
- Optimize imports **`Ctrl+Alt+O`**, **`Ctrl+Option+O`**
    - 전체 코드에서 자동으로 Optimize imports
        - Ctrl/Command+Shift+A → optimize imoprts on → ON
...
- Resume **`F9`**, **`Command+Option+R`**
    - 다음 BreakPoint로 이동 - BreakPoint에 의해 멈춘 라인은 아직 실행 되기 전
...
- Step Over **`F8`**, **`F8`** - 현재 BreakPoint가 걸려 있는 상태의 다음 줄로 이동
- Step Into **`F7`**, **`F7`** - 현재 BreakPoint가 걸려 있는 줄에서 다음에 실행될 메소드로 이동
- Step Out **`Shift+F8`**, **`Shift+F8`** - Step Into로 들어왔을 때, 다시 현재 BreakPoint로 Focus 이동
...
- Evaluate Expression **`Alt+F8`**, **`Option+F8`** - 현재 Break가 걸려있는 상태에서 코드 실행가능
...
>     - Watch - Variables에서 Break된 시점부터 다음 Break까지 실시간으로 값이 변하는 것을 확인
>     - 라이브러리/프레임워크를 분석할 때 매우 도움이 됨

## [7] Git&Github
### 1. Git 기본 기능 사용하기
- <ins>Git View On</ins> **`Alt+9`**, **`Command+9`** (View → Tool Windows → Version Control)
    - Local Changes - 최종 버전에서 어떤 파일들이 변경되었는지 확인 가능
        - Show Diff `Ctrl+D`, `Command+D` - 변경전/변경후, Diff 화면에서 비교하며 수정 가능
        - Revert `Ctrl+Alt+Z`, `Command+Option+Z` - 변경 내역을 취소
...
- <ins>기존 프로젝트에 Github 연동하기</ins>
    - Ctrl/Command+Shift+A → share github → Share Project on Github → Login to Github
...
## [8] 플러그인
- 플러그인 설치: Ctrl/Command+Shift+A → plugins → Plugins/Settings → sortBy:Downloads
- 추천 플러그인 
    ...
    - Translator: 강사(jojoldu)가 직접 제작한 번역 플러그인 `Alt+1/2/3`, `Option+1/2/3`
- <ins>Use Terminal</ins> **`Alt+F12`**, **`Option+F12`**
...
- **Community**: Java, Groovy, Gradle, Git 사용 가능
    - Web에 관련된 Servlet, Tomcat, Spring, Spring Boot, JSP, JS의 사용이 제한적
...
6. 프로젝트 생성 후 추가 설정
     ...
     - Ctrl/Command+Shift+A → plugins → Presentation Assistant → Install
